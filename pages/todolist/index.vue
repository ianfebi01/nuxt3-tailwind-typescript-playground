...
<template>
  <div class="w-full h-full flex flex-col">
    <MoleculesMHeaderTodolist :date="moment(new Date()).format('MMMM YYYY')" @click-button="modal = !modal" />
    <div class="mt-6 flex flex-col">
      <span>Filter by status</span>
      <div class="flex gap-4 py-4">
        <AtomsAButton :bg="filterBgColor(1)" color="gray - 800" font-weight="normal" :border="true" @click="filter(1)">
          <div class="flex items-center gap-2">
            <MoleculesMChecked :status="1" />
            <span>Done</span>
          </div>
        </AtomsAButton>
        <AtomsAButton :bg="filterBgColor(0)" color="gray-800" font-weight="normal" :border="true" @click="filter(0)">
          <div class="flex items-center gap-2">
    ...
    <div class="flex flex-col grow-[1] items-stretch mt-6">
      <OrganismsOListTodolist :datas="handleFilter" @change-status="changeStatus($event)" />
    </div>
    <Teleport to="body">
        ...
        <template #default>
          <div class="flex grow-[1] my-4 border-spacing-4 items-stretch">
            <div class="flex flex-col w-full gap-4">
              <InputDynamicField v-model="form[item?.valueName]" v-for="(item, i) in fields" :item="item" :error-messages="v$[item?.valueName].$invalid && v$[item?.valueName].$dirty
          ...
          <div class="w-full flex justify-end gap-2">
            <AtomsAButton :border="true" bg="white" color="blue-600" @click="modal = !modal">Cancel</AtomsAButton>
            <AtomsAButton bg="blue-600" :border="false" @click="handleSubmit">Add Task</AtomsAButton>
          </div>
        </template>
      </OrganismsOModal>
    </Teleport>
